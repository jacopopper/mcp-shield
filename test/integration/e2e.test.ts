...
describe('End-to-end integration', () => {
    describe('Client wrapping', () => {
        it('should wrap client and enforce policy', async () => {
            // Mock MCP client
            const callToolMock = mock.fn(async (name: string, args: unknown) => {
            ...
            await assert.rejects(
                ...
                (error: Error) => {
                    ...
                    assert.strictEqual((error as PolicyDeniedError).tool, 'dangerous.delete');
                    return true;
                }
            );
        ...
        it('should allow all tools with allow-all config', async () => {
            const callToolMock = mock.fn(async (_name: string, _args?: unknown) => ({ ok: true }));
            const mockClient = {
                callTool: callToolMock,
            };
